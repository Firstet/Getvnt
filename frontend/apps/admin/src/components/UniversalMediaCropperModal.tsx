...
export const UniversalMediaCropperModal: React.FC<Props> = ({
  ...
  return (
    <div className="modal-overlay" style={{ background: 'rgba(4, 6, 14, 0.90)', backdropFilter: 'blur(16px)', zIndex: 9999 }} onClick={onClose}>
      <div className="modal-box" style={{ maxWidth: '1100px', width: '94vw', background: '#0B0F19', border: '1px solid rgba(79,70,229,0.4)', borderRadius: '28px', padding: '32px', boxShadow: '0 32px 80px rgba(0,0,0,0.85)', maxHeight: '92vh', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
        ...
        {/* Tab 1: Editor Suite (70% Canvas / 30% Side Toolbar Layout) */}
        {activeTab === 'editor' && (
          <div>
            {!selectedImage ? (
              <div
                ...
                style={{
                  height: '320px', border: '2px dashed rgba(79,70,229,0.4)', borderRadius: '20px',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '28px', alignItems: 'start' }}>
                ...
                <div
                  ...
                  style={{
                    height: '420px', background: '#04060E', borderRadius: '20px', overflow: 'hidden',
                ...
                {/* ── RIGHT COLUMN (30%): SIDE TOOLS TOOLBAR PANEL ── */}
                <div style={{ background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '20px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '11px', fontWeight: 900, color: '#60A5FA', textTransform: 'uppercase', marginBottom: '10px', letterSpacing: '0.5px' }}>
                      CROP ASPECT RATIO
                    </label>
                      ...
                      {CROP_PRESETS.map((p) => (
                        <button
                          ...
                          style={{
                            ...
                            background: activePreset === p.id ? 'linear-gradient(135deg, #4F46E5, #06B6D4)' : 'rgba(255, 255, 255, 0.06)',
                            color: activePreset === p.id ? '#FFFFFF' : '#E5E7EB',
                            border: activePreset === p.id ? 'none' : '1px solid rgba(255, 255, 255, 0.12)',
                            borderRadius: '10px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'
                          }}
                          onClick={() => {
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                      <button type="button" className="btn-cta" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#FFF', padding: '8px 10px', fontSize: '11.5px', justifyContent: 'center' }} onClick={() => setRotation((r) => (r + 90) % 360)}>
                        <RotateCw size={13} /> Rotate
                      </button>
                      <button type="button" className="btn-cta" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#FFF', padding: '8px 10px', fontSize: '11.5px', justifyContent: 'center' }} onClick={() => setFlipH(!flipH)}>
                        <FlipHorizontal size={13} /> Flip H
                      </button>
                      <button type="button" className="btn-cta" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#FFF', padding: '8px 10px', fontSize: '11.5px', justifyContent: 'center' }} onClick={() => setFlipV(!flipV)}>
                        <FlipVertical size={13} /> Flip V
                      </button>
                      <button type="button" className="btn-cta" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#FFF', padding: '8px 10px', fontSize: '11.5px', justifyContent: 'center' }} onClick={handleResetTransforms}>
                  ...
                  {/* Actions */}
                  <div style={{ borderTop: '1px dashed rgba(255,255,255,0.1)', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <button
                      type="button"
                      disabled={isUploading}
                      className="btn-cta"
                      style={{ width: '100%', background: 'linear-gradient(135deg,#4F46E5,#06B6D4)', color: '#FFF', justifyContent: 'center', fontWeight: 900, padding: '12px', borderRadius: '12px', fontSize: '14px' }}
                      onClick={handleUpload}
                    >
                      {isUploading ? 'Saving Asset...' : 'Upload Media'} <Check size={16} />
                    </button>

                    <button type="button" style={{ background: 'none', border: 'none', color: '#9CA3AF', fontSize: '11.5px', cursor: 'pointer', textAlign: 'center' }} onClick={() => { setSelectedImage(null); setCropRect(null); }}>
